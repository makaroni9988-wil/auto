...
input string CustomLevels        = "50;30;70"; // Separate levels with a semicolon (e.g. 30;50;70)

input group "--- DIVERGENCE SETTINGS ---"

input bool   EnableDivergence         = false;

// Divergence calculated from the RSI line only
...
input bool   DrawRSIDivergence        = true;
...
PlotIndexSetInteger(0, PLOT_DRAW_TYPE, DRAW_LINE);
   ...
   {
      Print("Div-RSI ERROR: failed to create internal RSI handle, error code ", GetLastError(), ".");
      return(INIT_FAILED);
   }
...
if(checkLevels)
{
   ...
   {
      Print("Div-RSI ERROR: BarsCalculated(rsiHandle) failed, error code ", GetLastError(), ".");
      g_loggedNoBars = true;
   }
   else if(barsReady == 0 && !StillWarmingUp() && !g_loggedNoBars)
   {
      ...
      Print("Div-RSI: RSI data isn't ready yet after warm-up. Will keep retrying..");
      g_loggedNoBars = true;
   }
   else if(barsReady > 0)
   ...
   // SetIndexBuffer() auto-marks RSIBuffer as a timeseries
   // (index 0 = newest bar), but time[]/high[]/low[] here are in
   // chronological order (index 0 = oldest bar) since ArraySetAsSeries
   // was never called on them. Without this, RSIBuffer[i] and time[i]
   ...
   // 100k+) from the RSI handle on EVERY SINGLE TICK.
   int toCopy;
   ...
   {
      Print("Div-RSI ERROR: CopyBuffer() failed for RSI (copiedRSI=", copiedRSI,
...
void DrawCustomLevel(string name, double priceValue, int windowIndex)
{
   ...
   {
      Print("Div-RSI ERROR: failed to create level line '", name, "', error code ", GetLastError(), ".");
      return;
   }
...
//+------------------------------------------------------------------+
// Notes:
// - Divergence is calculated from the RSI line only.
// - Uses confirmed RSI pivots.
...
// - DrawRSIDivergence draws inside this RSI subwindow.
                     ...
                     int subWindow)
{
   ...
   // --- keep pivot scanning out of the RSI warm-up region ---
   // The first valid RSI value needs roughly RSIPeriod bars of history;
   // before that the RSI buffer can still hold EMPTY_VALUE placeholders,
   ...
   int warmup = RSIPeriod + PivotLeftBars;
   if(start < warmup)
      start = warmup;
   ...
   for(int i = start; i <= end; i++)
   {
      ...
      if(lastHighPivot >= 0)
      {
         bool bearDiv =
            ...
            RSIBuffer[i] < RSIBuffer[lastHighPivot];
         ...
         if(bearDiv)
         {
            ...
            bool passesOBOS =
               ...
               (RSIBuffer[bearRunAnchor] >= BearDivMinLevel ||
                RSIBuffer[i]             >= BearDivMinLevel);
   ...
   for(int i = start; i <= end; i++)
   {
      ...
      if(lastLowPivot >= 0)
      {
         bool bullDiv =
            ...
            RSIBuffer[i] > RSIBuffer[lastLowPivot];
         ...
         if(bullDiv)
         {
            ...
            bool passesOBOS =
               !RequireOBOSForDivergence ||
               (RSIBuffer[bullRunAnchor] <= BullDivMaxLevel ||
                RSIBuffer[i]             <= BullDivMaxLevel);
...
//+------------------------------------------------------------------+
//| Check If RSI High Pivot                                          |
//+------------------------------------------------------------------+
bool IsRSIHighPivot(int index)
{
   for(int i = 1; i <= PivotLeftBars; i++)
   {
      if(RSIBuffer[index] <= RSIBuffer[index - i])
         return false;
   }
...
//+------------------------------------------------------------------+
//| Check If RSI Low Pivot                                           |
//+------------------------------------------------------------------+
bool IsRSILowPivot(int index)
{
   for(int i = 1; i <= PivotLeftBars; i++)
   {
      if(RSIBuffer[index] >= RSIBuffer[index - i])
                        ...
                        int subWindow)
{
   ...
   DivLine indLines[];   // lines drawn in this RSI subwindow this pass
   ...
   for(int r = first; r < total; r++)
   {
      ...
      if(DrawRSIDivergence)
      {
         ...
         AppendLine(indLines, name, time[a], RSIBuffer[a],
                    time[e], RSIBuffer[e], lineColor, thickness, subWindow);
      }
                          ...
                          int windowIndex)
{
   ...
   {
      Print("Div-RSI ERROR: failed to create divergence line '", name, "', error code ", GetLastError(), ".");
      return;
   }
